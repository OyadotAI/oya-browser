...
const { isConfigured } = await import('../../src/drivers/sandbox.ts');
...
try {
  ...
  assert(
    isConfigured({ OYA_CLOUD_API_KEY: 'a', OYA_CLOUD_SNAPSHOT: 'b' }) === false,
  ...
  const { removeSandbox } = await import('../../src/drivers/sandbox.ts');
  let rejected = false;
  try {
    await removeSandbox('some-id', '');
  } catch (e) {
  ...
  assert(
    gemini.baseUrl === 'https://generativelanguage.googleapis.com/v1beta/openai' && gemini.model === 'gemini-3.8-flash',
  ...
  assert(
    vertex.baseUrl === 'https://aiplatform.googleapis.com/v1/publishers/google' && vertex.model === 'gemini-2.5-flash',
  ...
  let refused = null;
  try {
    await keyConfig.set('k-vertex', { llm_provider: 'vertx' });
  } catch (e) {
    refused = e;
  }
  assert(
    refused?.status === 400 && /openai, anthropic, gemini, vertex/.test(refused.message),
    'an unknown llm_provider is refused with the list of valid ones',
  );
  assert(
    keyConfig.resolve('k-vertex').model === 'gemini-2.5-flash',
    'the refused write left the previous provider in place',
  );
  for (const [field, value] of [
    ['browser_provider', 'not-a-provider'],
    ['captcha_solver', 'not-a-solver'],
  ]) {
    let bad = null;
    try {
      await keyConfig.set('k-vertex', { [field]: value });
    } catch (e) {
      bad = e;
    }
  ...
  for (const id of keyConfig.PROVIDER_CHOICES.map((p) => p.id))
    await keyConfig.set('k-choices', { browser_provider: id });
  ...
  assert(
    keyConfig.get('k-blank').openai_base_url === 'https://server-wide.test/v1',
    'a blank own base URL is ignored by get()',
  );
  assert(
    keyConfig.resolve('k-blank').baseUrl === 'https://server-wide.test/v1',
  ...
  assert(
    !shownSecret.includes('supersecret') && shownSecret.endsWith('tail'),
    'a stored LLM credential reads back masked',
  );
  assert(keyConfig.resolve('k-secret').openaiKey === 'sk-supersecret-tail', 'the server still resolves the plaintext');
  assert(
    keyConfig.envFor('k-secret').OPENAI_API_KEY === 'sk-supersecret-tail',
  ...
  assert(
    keyConfig.resolve('k-secret').openaiKey === 'sk-supersecret-tail',
    'saving the masked placeholder leaves the real credential intact',
  );

  // One key's settings are invisible to another.
  assert(keyConfig.resolve('k-other').openaiKey === serverKey, "another key sees none of this key's settings");
  ...
  for (const bad of blocked) {
    try {
      await keyConfig.set('k-ssrf', { openai_base_url: bad });
    } catch (e) {
      if (e.status === 400) blockedCount++;
      else console.log('    unexpected:', bad, e.message);
    }
  }
  ...
  assert(
    ['../../src/modules/agent/chat.ts', '../../src/modules/playbooks/service.ts'].every(
      (f) => !/fetch\(/.test(read(f)),
    ),
  ...
  Daytona.prototype.list = async function* () {
    if (!removed) yield cloud;
  };
  Daytona.prototype.get = async function () {
    return cloud;
  };
  const disconnected = await call('/browsers', 'GET', undefined, 'tenant-key');
  assert(
    disconnected.body.some((b) => b.id === 'cloud-browser' && b.health === 'dead'),
    'running sandbox remains listed without a WebSocket',
  );
  ...
  assert(
    detail.status === 200 && detail.body.lastError?.includes('disconnected'),
    'disconnected sandbox explains its state in the detail panel',
  );
  const fleet = await call('/fleet', 'GET', undefined, 'tenant-key');
  assert(
    fleet.body.browsers.total === 1 && fleet.body.browsers.byHealth.dead === 1,
    'fleet counters include disconnected sandboxes',
  );
  const { listSandboxBrowsers } = await import('../../src/drivers/sandbox.ts');
  const reconnected = await listSandboxBrowsers('tenant-key', [
    { id: 'cloud-browser', health: 'ok', name: 'Connected again' },
  ]);
  assert(
    reconnected.length === 1 && reconnected[0].health === 'ok',
    'reconnection replaces the disconnected row without duplication',
  );
  ...
  assert(
    !afterStop.body.some((b) => b.id === 'cloud-browser'),
    'stopped sandbox disappears immediately from inventory',
  );
    ...
    return {
      ...
      process: {
        ...
        getEntrypointSession: async () => ({
          commands: [{ command: autoEntrypoint ? "'/docker-entrypoint.sh'" : 'sleep infinity' }],
        }),
        createSession: async () => {},
  ...
  const { createSandbox } = await import('../../src/drivers/sandbox.ts');
