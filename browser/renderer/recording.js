(() => {
  ...
  $('record-save').addEventListener('click', async () => { if (busy) return; const name = $('record-name').value.trim(), description = $('record-desc').value || name; busy = true; render(state); try { if (!/^[\w-]{1,64}$/.test(name)) throw new Error('Use 1–64 letters, numbers, hyphens or underscores for the published name.'); const result = await oyaBrowser.saveRecording(name, description); if (result.error) throw new Error(result.error); say(`Playbook “${name}” saved to Oya. Your local draft is retained.`); await command({ type: 'get' }); } catch (error) { say(error.message, true); } finally { busy = false; render(state); } });
