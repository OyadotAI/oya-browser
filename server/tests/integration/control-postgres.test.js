...
const sql = async (query) =>
  (
    await exec('psql', ['-XqAt', '-v', 'ON_ERROR_STOP=1', '-d', process.env.PGDATABASE || 'postgres', '-c', query], {
...
const remote = {
  async rpc(name, args = {}) {
    try {
      const raw = await sql(
        `set role service_role; select oya_browser.${name}(${Object.entries(args)
          .map(([k, v]) => `${k} => ${literal(v)}`)
          .join(', ')});`,
      );
      return { data: raw === '' ? null : JSON.parse(raw) };
    } catch (error) {
      return { error: { message: String(error.stderr || error.message) } };
    }
  },
};
...
const attempts = await Promise.allSettled(
  Array.from({ length: 12 }, (_, i) => (i % 2 ? a : b).reserve(key, { provider: 'cdp', maxConcurrent: 1 })),
);
...
const idempotent = await Promise.all(
  [a, b].map((c) => c.reserve(key, { provider: 'cdp', idempotencyKey: 'same', request: { name: "quoted ' safely" } })),
);
...
assert.equal(
  (await a.store.events({ seqs: [delivery.eventSeq] }))[0].type,
  'project.settings.updated',
  'deliveries are created atomically with their event',
);
...
console.log(
  'Postgres contracts passed: migration, service-role RPC, replica admission race, takeover exclusion, revocation, idempotency, event fan-out, cursor, retention, anonymous denial.',
);
