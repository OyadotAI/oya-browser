...
import { allowedHost } from '../../src/modules/control/egress.ts';
import { validatePolicy } from '../../src/modules/control/service.ts';

/** The pre-wildcard matcher, kept verbatim as the oracle. */
const previous = (host, rules) => {
  const normalized = host.toLowerCase().replace(/\.$/, '');
  return rules.some((rule) =>
    rule.startsWith('*.') ? normalized.endsWith(rule.slice(1)) && normalized !== rule.slice(2) : normalized === rule,
  );
};
...
assert.equal(
  allowedHost('us-central1-aiplatform.googleapis.com', vertex),
  true,
  'mid-label wildcard matches a region host',
);
assert.equal(
  allowedHost('europe-west4-aiplatform.googleapis.com', vertex),
  true,
  'mid-label wildcard matches another region',
);
...
rejects(
  { allowedHosts: ['*'.repeat(12) + 'x.example.com'] },
  'a rule with 12 stars is refused before it can backtrack',
);
