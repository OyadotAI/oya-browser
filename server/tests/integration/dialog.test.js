...
assert(
  !AUTO_ACCEPT.has('confirm') && !AUTO_ACCEPT.has('prompt'),
  'a confirm or prompt is a decision, so it is never answered automatically',
);
assert(
  describe({ type: 'alert', message: 'Invalid login' }, true).includes('Invalid login'),
  'the reported note carries the real message, not a fixed string',
);
assert(
  /handle_dialog/.test(describe({ type: 'confirm', message: 'Delete?' })),
  'a held dialog tells the caller how to answer it',
);
assert(
  describe({ type: 'prompt', message: 'Name?', defaultPrompt: 'Ada' }).includes('Ada'),
...
try {
  ...
  assert(
    String(clicked.data?.dialog || '').includes('Invalid login'),
  ...
  assert(
    blocked.ok === false && /confirm dialog is open/.test(blocked.error),
  ...
  assert(
    (await driver.send('handle_dialog', { accept: true })).ok === false,
