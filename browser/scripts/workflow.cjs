...
function candidates(el = {}) {
  const out = [];
  const byAttribute = [];
  if (el.domId) byAttribute.push({ kind: 'css', value: `[id=${JSON.stringify(el.domId)}]` });
  if (el.name) byAttribute.push({ kind: 'css', value: `[name=${JSON.stringify(el.name)}]` });
  if (el.href) byAttribute.push({ kind: 'css', value: `a[href=${JSON.stringify(el.href)}]` });
  if (el.testId) out.push({ kind: 'testId', value: el.testId });
  if (FIELD.has(el.type)) out.push(...byAttribute);
