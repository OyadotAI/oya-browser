...
const { sanitizeSteps, templateValues, renderPlaywright, variablesOf } =
...
try {
  ...
  const pb = templateValues({
    name: 'acceptance',
    defaults: {},
    secrets: recording.data.secrets,
    steps: sanitizeSteps(recording.data.steps),
  });
  ...
  await new Function(renderPlaywright(withoutSubmit).replace('export default ', 'return '))()(page, {
    password: 'replay-secret',
  });
  assert.equal(await page.locator('#editor').innerText(), 'Editable description');
  console.log(
    'recording export: real capture, hidden/custom controls, navigation, secrets, defaults and overrides replayed successfully',
  );
} finally {
  driver?.close();
  await browser?.close();
