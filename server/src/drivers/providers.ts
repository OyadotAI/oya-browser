...
import { CDP_DISCOVERY_TIMEOUT_MS, PROVIDER_RELEASE_TIMEOUT_MS } from './constants.ts';
...
/**
 * A CDP endpoint the caller already has: either the WebSocket URL, or the
 * http address Chrome prints for --remote-debugging-port, which is what people
 * have in front of them. The latter is resolved through Chrome's own
 * /json/version, and the answer must point at the same host and port that was
 * checked by the network guard, so a rogue endpoint cannot redirect us.
 */
async function direct(wsUrl, provider) {
  if (!wsUrl) fail('wsUrl is required for the cdp provider');
  const parsed = parseUrl(wsUrl);
  if (!parsed) fail('wsUrl is not a valid URL');
  if (isDiscovery(parsed)) return direct(await discover(parsed), provider);
  if (!isWebSocket(parsed)) fail('wsUrl must be ws://, wss://, or the http address of a debugging port');
  return { wsUrl, provider, sessionId: null, release: async () => {} };
}

/** An http address, which Chrome serves its discovery document on. */
const isDiscovery = (url) => ['http:', 'https:'].includes(url.protocol);

/** Chrome's discovery document, or a failure that names the address that did not answer. */
async function discoveryDocument(endpoint: string) {
  const answer = await fetch(endpoint, { signal: AbortSignal.timeout(CDP_DISCOVERY_TIMEOUT_MS) }).catch((err) =>
    fail(`No debugging port answered at ${endpoint}: ${err.message}`),
  );
  if (!answer.ok) fail(`${endpoint} answered ${answer.status}, so it is not a Chrome debugging port`);
  return (await answer.json().catch(() => ({}))) as {
    /** The address Chrome wants CDP clients to dial. */
    webSocketDebuggerUrl?: string;
  };
}

/** The WebSocket URL Chrome names at /json/version, refused if it points anywhere else. */
async function discover(url) {
  const endpoint = new URL('/json/version', url).href;
  const found = parseUrl((await discoveryDocument(endpoint)).webSocketDebuggerUrl || '');
  if (!found) fail(`${endpoint} named no webSocketDebuggerUrl`);
  if (found.host !== url.host) fail(`${endpoint} pointed at ${found.host}, not itself`);
  return found.href;
}
