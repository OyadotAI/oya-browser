...
Object.assign(process.env, { OYA_DATA_DIR: directory, OYA_PROFILE_SECRET: 'desktop-contract', API_KEYS: 'desktop-owner' });
delete process.env.SUPABASE_URL; delete process.env.SUPABASE_SERVICE_KEY;
const { control } = await import('./src/control/service.js');
const { desktopControl, desktopHolder } = await import('./src/control/desktop.js');
const { handleConnection } = await import('./src/ws-handler.js');
const service = control(), key = 'desktop-owner', id = randomUUID();
...
try {
  ...
  while ((await service.store.get('session', id)).control.mode === 'agent') await new Promise(resolve => setTimeout(resolve, 5));
  ...
  await assert.rejects(service.beginCommand(id), { code: 'control_paused' });
  const humanCommand = await service.beginCommand(id, desktopHolder(id)); await humanCommand();
  ...
  const resumed = await service.beginCommand(id); await resumed();
  ...
  assert.equal((await desktopControl(key, id, 'return')).mode, 'agent', 'expired/released leases can explicitly resume');
  await desktopControl(key, id, 'acquire');
  await service.store.transact(async tx => { (await tx.get('session', id)).control.expiresAt = Date.now() - 1; });
  ...
  const messages = [], wait = predicate => new Promise((resolve, reject) => {
    const started = Date.now();
    const tick = () => { const index = messages.findIndex(predicate); if (index >= 0) return resolve(messages.splice(index, 1)[0]); if (Date.now() - started > 12000) return reject(new Error('Timed out waiting for control response')); setTimeout(tick, 5); }; tick();
  });
  ...
  const request = async (action, extra = {}) => {
    const requestId = randomUUID(); client.send(JSON.stringify({ type: 'desktop_control', id: requestId, action, ...extra }));
    return wait(m => m.id === requestId);
  };
  const command = await request('command-start'); assert(command.token);
  ...
  const outstanding = await request('command-start'); assert(outstanding.token);
  ...
  console.log('Desktop control passed: admission drain, ownership, renewal, expiry, return, real WebSocket handoff and local CDP gates.');
} finally {
  client?.terminate(); for (const ws of wss.clients) ws.terminate();
  await new Promise(resolve => wss.close(resolve));
  await service.store.close(); await rm(directory, { recursive: true, force: true });
}
