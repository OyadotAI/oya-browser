...
describe('site credentials', () => {
  ...
  it('looks up a subdomain through its registrable parent', () => {
    credentials.set('p-1', 'evicore.com', LOGIN);
    assert.deepEqual(credentials.lookup('p-1', 'carecore.evicore.com'), { domain: 'evicore.com', ...LOGIN });
  });

  it('prefers a credential filed under the exact host', () => {
    credentials.set('p-1', 'evicore.com', LOGIN);
    credentials.set('p-1', 'carecore.evicore.com', { username: 'exact', password: 'p' });
    assert.equal(credentials.lookup('p-1', 'carecore.evicore.com').username, 'exact');
  });
