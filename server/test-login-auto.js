...
const { removeScratch } = await import('./test-support/scratch.js');
...
credentials.set(PERSONA, 'https://www.radmd.com/RadMD/Common/Login.aspx', { username: 'alice', password: 'hunter2-secret' });
credentials.set(PERSONA, 'evicore.com', { username: 'bob', password: 'correct-horse' });

const described = credentials.describe(PERSONA, 'radmd.com');
...
assert(credentials.domainOf('https://www.radmd.com/x') === 'radmd.com', 'a URL files under its host without www');
...
assert(credentials.lookup(PERSONA, 'radmd.com').password === 'hunter2-secret', 'the server can still open it');
assert(credentials.lookup(PERSONA, 'carecore.evicore.com')?.username === 'bob', 'a subdomain falls back to the registrable parent');
assert(credentials.lookup(OTHER, 'radmd.com') === null, 'another persona sees nothing');

credentials.restore();
assert(credentials.lookup(PERSONA, 'radmd.com')?.username === 'alice', 'survives a reload from disk');

console.log('\n2️⃣  A factor per site, with the persona-wide one as fallback...');
const SEED = 'JBSWY3DPEHPK3PXP';
await mfa.set(PERSONA, { type: 'totp', secret: SEED });                       // persona-wide
await mfa.set(PERSONA, { type: 'totp', secret: 'KRSXG5CTMVRXEZLU' }, 'evicore.com');
assert(mfa.describe(PERSONA, 'evicore.com').domain === 'evicore.com', 'the site factor wins for its own site');
assert(mfa.describe(PERSONA, 'radmd.com').configured && !mfa.describe(PERSONA, 'radmd.com').domain,
...
assert(mfa.list(PERSONA).length === 1 && mfa.list(PERSONA)[0].domain === 'evicore.com', 'list() shows only site factors');
assert(!readFileSync(join(process.env.OYA_DATA_DIR, 'mfa.json'), 'utf8').includes('JBSW'), 'no plaintext seed reaches the disk');

console.log('\n3️⃣  Mailbox parsing, no network...');
const b64url = (s) => Buffer.from(s, 'utf8').toString('base64').replace(/\+/g, '-').replace(/\//g, '_');
assert(inbox.gmailBody({ mimeType: 'multipart/alternative', parts: [
  { mimeType: 'text/html', body: { data: b64url('<p>Your code is <b>445566</b></p>') } },
  { mimeType: 'text/plain', body: { data: b64url('Your code is 112233') } },
]}).includes('112233'), 'gmail prefers the plain-text part');
assert(inbox.gmailBody({ mimeType: 'text/html', body: { data: b64url('<style>x{}</style><p>code 778899</p>') } })
  === 'code 778899', 'an html-only body is reduced to its text');
assert(inbox.htmlToText('<p>a&nbsp;&amp;&nbsp;b</p>') === 'a & b', 'entities decode');

console.log('\n4️⃣  The code is read by the LLM, not a regex...');
{
  const EMAIL = [
    'Carelon Medical Benefits Management: verification',
  ...
  assert(await mfa.extractCode(EMAIL, { llm }) === 'K7R4QP', 'the LLM picks the code out of a message full of other numbers');
  ...
  const chatty = createServer((req, res) => {
    req.resume();
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ choices: [{ message: { content: 'The verification code is 998877, valid for five minutes.' } }] }));
  });
  await new Promise((r) => chatty.listen(0, '127.0.0.1', r));
  const chattyLlm = { ...llm, baseUrl: `http://127.0.0.1:${chatty.address().port}` };
  assert(await mfa.extractCode('code 112233', { llm: chattyLlm }) === '112233',
  ...
  assert(await mfa.extractCode('Your invoice 445566 is ready.', { llm: { ...llm, baseUrl: `http://127.0.0.1:${refusing.address().port}` } }) === null,
    'NONE means no code, rather than grabbing the nearest number');

  const broken = createServer((req, res) => { req.resume(); res.writeHead(500); res.end('nope'); });
  await new Promise((r) => broken.listen(0, '127.0.0.1', r));
  assert(await mfa.extractCode('code 334455', { llm: { ...llm, baseUrl: `http://127.0.0.1:${broken.address().port}` } }) === '334455',
...
console.log('\n6️⃣  A code from the previous run is never reused...');
{
  ...
  const relay = createServer((req, res) => {
    ...
    res.writeHead(200, { 'Content-Type': 'text/plain', 'x-oya-received-at': String(Date.now() - (stale ? 600_000 : 0)) });
  ...
  const fake = (expr) => {
    if (expr.includes('data-oya-mfa-target')) {
      ...
      if (m) { typed = m[1]; return { filled: true }; }
    }
    if (expr.startsWith('(() => {\n  const fields')) return { present: !typed, segmented: false, fieldCount: 1 };
    return false;
  };
...
const site = createServer((req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    req.on('end', () => {
      ...
      if (url.pathname === '/login' && ok) {
        ...
        // button. See qa/carelon-portal/test-cases.md step 2.
        return res.end('<!doctype html><title>portal</title><p>We will send a code to a\u2022\u2022\u2022\u2022e@example.com. It is valid for five minutes.</p><button id="send">Send Email</button>');
      }
      res.writeHead(200, { 'Content-Type': 'text/html' });
  ...
  if (url.pathname === '/code') return res.end('<!doctype html><title>portal</title><input autocomplete="one-time-code" maxlength="6"><button>Verify Code</button>');
  ...
  if (url.pathname === '/contact') return res.end('<!doctype html><title>portal</title><h1>Contact your account manager</h1><p>We usually reply within two business days.</p><button>Send Email</button>');
  res.end(form());
});
...
try {
  ...
  assert(await driver.evaluateMain('document.querySelector("h1").textContent') === 'Contact your account manager',
