...
const slack = await import('../../src/modules/slack/service.ts');
const { control } = await import('../../src/modules/control/service.ts');
const { deliver } = await import('../../src/modules/control/worker.ts');
...

{
  const body = 'payload=%7B%7D',
    ts = now();
  assert.equal(
    slack.verifySignature(body, { 'x-slack-request-timestamp': ts, 'x-slack-signature': sign(body, ts) }),
    true,
    'a correct signature verifies',
  );
  assert.equal(
    slack.verifySignature(body, { 'x-slack-request-timestamp': ts, 'x-slack-signature': sign(body, ts, 'wrong') }),
    false,
    'another secret is rejected',
  );
  assert.equal(
    slack.verifySignature('payload=%7B%22a%22%3A1%7D', {
  ...
  assert.equal(
    slack.verifySignature(body, { 'x-slack-request-timestamp': stale, 'x-slack-signature': sign(body, stale) }),
    false,
...

{
  const attention = {
    ...
    sessionId: 'brw-1',
    detail: { runId: 'run_9', owner: 'own', reason: 'captcha', message: 'Solve it in the live view.' },
  };
  ...
  assert.deepEqual(
    JSON.parse(buttons[1].value),
    { runId: 'run_9', owner: 'own' },
    'the resume button round-trips what respond() needs',
  );

  const failed = slack.blocksFor(
    { type: 'run.failed', sessionId: 'brw-1', detail: { runId: 'run_9', error: 'Step limit' } },
    null,
  );
...
globalThis.fetch = async (url, options) => {
  ...
  posted.push({
    url: String(url),
    body: typeof raw === 'string' ? JSON.parse(raw) : Object.fromEntries(new URLSearchParams(String(raw))),
  });
...
{
  const deliveries = await emitAndDeliver('run.needs_attention', {
    runId: 'run_1',
    owner: 'own',
    reason: 'mfa',
    message: 'MFA needs completing.',
  });
  ...
  assert.match(
    link,
    /^https:\/\/console\.example\.com\/live\/[^#]+#t=oya_/,
    'the message carries a share link for that browser',
  );
  assert.equal(
    deliveries.every((d) => d.state === 'delivered'),
    true,
    'a successful post settles the delivery',
  );
}
...

{
  ...
  const callback = (state, cookie) =>
    fetch(`${base}/api/slack/callback?code=abc&state=${encodeURIComponent(state)}`, {
      redirect: 'manual',
  ...
  try {
    ...
    assert.match(
      String(
        await fetch(`${base}/api/slack/install?json=1`, { headers: auth }).then((r) => r.headers.get('set-cookie')),
      ),
      /HttpOnly/i,
    );
    ...
    assert.equal(
      ...
      '/dashboard?slack=state_mismatch',
      'a state replayed in another browser is refused',
    );
    ...
    assert.equal(
      ...
      '/dashboard?slack=state_mismatch',
      'a mismatched cookie is refused too',
    );
    ...
    assert.equal(
      ...
      '/dashboard?slack=connected',
      'an install that kept its channel goes straight back',
    );
