...
export function renderPlaywright(pb) {
  ...
  const defaults = Object.fromEntries(Object.entries(pb.defaults || {}).filter(([key]) => vars.includes(key) && !secrets.has(key)));
  const lines = pb.steps.map((step) => {
    switch (step.action) {
      ...
      case 'handle_dialog': return `// dialog answered by the handler above`;
  ...
  const dialogs = pb.steps.filter((step) => step.action === 'handle_dialog' && step.accept !== false);
  return [
    ...
    ...(dialogs.length ? [`  page.on('dialog', (d) => d.accept(${dialogs.some((step) => step.prompt_text) ? s(dialogs.find((step) => step.prompt_text).prompt_text) : ''}));`] : []),
    ...lines.map((l) => `  ${l}`),
    `}`,
    '',
  ].join('\n');
...
async function runStep(browserId, step, values, defaults = {}) {
  switch (step.action) {
    ...
    case 'handle_dialog': return command(browserId, 'handle_dialog', { accept: step.accept !== false, prompt_text: step.prompt_text });
