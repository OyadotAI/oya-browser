...
- **`sdk/src/browser.ts`, class `Browser`** is over the 200-line class limit,
  ...
  interface merge).
- **`cli/src/install/repo.ts`** still recognises a checkout by
  `server/src/index.js`. The server's entry point is now `index.ts`, so on a
  current checkout `oya install` will not find the repo. The behavior was kept
  as it was, for a maintainer to fix on purpose. The integration test's fake
  checkout still creates `index.js`.
