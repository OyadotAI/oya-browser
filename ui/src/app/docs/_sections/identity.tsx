...
API key = a group of personas                    # your fleet`}</CodeBlock>
      <p className="mb-3 text-[15px] leading-relaxed">
        A persona&apos;s fingerprint is derived from a stored seed, so it is byte-identical across restarts, a returning
        session looks like a returning device, not a new one.
      </p>
...
if (!r.completed) open(r.liveViewUrl);   // finish it by hand`}</CodeBlock>
      <p className="mb-3 text-[15px] leading-relaxed">
        ...
        within a bounded window, because the code does not exist yet when the prompt appears. When nothing automated can
