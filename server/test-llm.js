...
import { toGemini, fromGemini, chatCompletion } from './src/llm.js';
import { BROWSER_TOOLS } from './src/chat-tools.js';
import * as keyConfig from './src/key-config.js';
...
assert.deepEqual(request.systemInstruction, { parts: [{ text: 'be brief' }] }, 'a system message becomes systemInstruction');
assert.deepEqual(request.contents, [{ role: 'user', parts: [{ text: 'go to example.com' }] }], 'a user message becomes a user content');
...
assert.ok(!('additionalProperties' in declaration.parameters), 'additionalProperties is stripped from a real tool schema');
...
assert.ok(!('systemInstruction' in toGemini({ messages: [{ role: 'user', content: 'hi' }] })), 'no system message means no systemInstruction');
...
assert.deepEqual(JSON.parse(message.tool_calls[0].function.arguments), { url: 'https://example.com' }, 'args are serialised as OpenAI arguments');
...
assert.ok(!('tool_calls' in fromGemini({ candidates: [{ content: { parts: [{ text: 'done' }] } }] }).choices[0].message), 'a text-only reply has no tool_calls');

// ── The round trip that actually breaks runs ──

const next = toGemini({ messages: [
  { role: 'user', content: 'go to example.com' },
...
assert.deepEqual(next.contents[2], { role: 'user', parts: [{ functionResponse: { name: 'navigate', response: { result: 'Navigated.' } } }] },
  'a tool result maps back to the function it answers');

// Several results from one assistant turn belong in a single content.
const parallel = toGemini({ messages: [
...
assert.deepEqual(parallel.contents[1].parts.map((p) => p.functionResponse.name), ['navigate', 'analyze_page'], 'each result keeps its own function');

// Malformed arguments must not take the process down mid-run.
assert.deepEqual(toGemini({ messages: [{ role: 'assistant', tool_calls: [{ id: 'navigate:0', function: { name: 'navigate', arguments: 'not json' } }] }] })
  .contents[0].parts[0].functionCall.args, {}, 'unparseable arguments degrade to empty args');
...
const native = await chatCompletion({ baseUrl: `${origin}/v1/publishers/google`, apiKey: 'express-key', model: 'gemini-2.5-flash', messages });
assert.equal(native.choices[0].message.content, 'ok', 'the express-mode reply is translated back');
assert.equal(seen[0].url, '/v1/publishers/google/models/gemini-2.5-flash:generateContent?key=express-key',
...
assert.ok(!seen[0].headers.authorization && !seen[0].headers['x-goog-api-key'], 'express mode sends no credential header');
assert.ok(seen[0].body.contents, 'express mode sends the native body');

await chatCompletion({ baseUrl: `${origin}/v1/projects/p/locations/l/endpoints/openapi`, apiKey: 'oauth-token', model: 'google/gemini-2.5-flash', messages });
assert.equal(seen[1].url, '/v1/projects/p/locations/l/endpoints/openapi/chat/completions', 'a project-scoped endpoint keeps the OpenAI path');
...
assert.equal(seen[2].url, '/v1/publishers/google/models/m:generateContent?key=k', 'a trailing slash on the base URL is tolerated');
...
seen.length = 0;
await chatCompletion({ ...configured, baseUrl: configured.baseUrl.replace('https://aiplatform.googleapis.com', origin), apiKey: 'k', messages });
assert.ok(seen[0].url.includes(':generateContent?key='), `the vertex preset (${configured.baseUrl}) routes to the native endpoint`);
...
if (express || studio) {
  ...
  const call = express
    ? (messages) => chatCompletion({ baseUrl: preset.base, apiKey: express, model: preset.model, messages, tools: [navigate] })
    : async (messages) => {
        ...
        if (!res.ok) assert.fail(`live: Gemini rejected the translated request (${res.status}: ${(await res.text()).slice(0, 300)})`);
  ...
  assert.ok(calls?.length, `live: the model asked for a tool call (got ${JSON.stringify(first.choices[0].message).slice(0, 200)})`);
  assert.equal(calls[0].function.name, 'navigate', 'live: it picked the tool we declared');
  assert.equal(JSON.parse(calls[0].function.arguments).url, 'https://example.com', 'live: the arguments survived translation');
  ...
  const second = await call([
    ...
    ...calls.map((c) => ({ role: 'tool', tool_call_id: c.id, content: 'Navigated to https://example.com. The page title is "Example Domain".' })),
  ...
  console.log(`  live ${express ? `express mode (${preset.base})` : 'AI Studio native'} round trip ok — ${JSON.stringify(second.choices[0].message.content).slice(0, 80)}`);
} else {
  console.log('  (set VERTEX_EXPRESS_KEY, or GEMINI_API_KEY, for the live round-trip check)');
}

console.log('LLM translation passed: system instruction, schema sanitising, tool calls, thought signatures, tool-result mapping, parallel results, usage metering, express-mode vs project-scoped routing.');
