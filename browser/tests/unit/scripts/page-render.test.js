...
describe('createRenderer', () => {
  ...
  it('gives every renderer the same interface', () => {
    for (const format of FORMATS) {
      ...
      for (const method of ['render', 'fit', 'forAgent'])
...
describe('markdown renderer', () => {
  ...
  it('gives the agent the page and the element index, the index whole even when the page is cut', () => {
    const text = md.forAgent(analysis(), 400);
    assert.ok(text.includes('## Element Index (2 total, 2 visible)'));
    assert.ok(text.includes('2,email,Email,a@b.test'));
    assert.ok(text.includes('⚠ Output truncated to fit context window.'));
  });
});
