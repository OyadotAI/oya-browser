...
(async () => {
  let server = { mode: 'agent', revision: 1 }, failAcquire = false;
  const sent = [];
  const controller = createControlState({ changed() {}, send(message) {
    sent.push(message);
    queueMicrotask(() => {
      ...
      if (message.action === 'acquire' && failAcquire) return controller.result({ id: message.id, error: 'Command still running', state: server });
  ...
  try {
    ...
    console.log('Control state passed: local drain, shared gate tokens, conflicting requests, stale updates, failed takeover, disconnect and older servers.');
