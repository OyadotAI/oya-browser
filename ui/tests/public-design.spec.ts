...
test('landing is readable at desktop and mobile widths without changing the app theme', async ({ page }, testInfo) => {
  ...
  expect(await page.locator('main').evaluate(el => getComputedStyle(el.parentElement!.parentElement!).backgroundColor)).toBe('rgb(255, 255, 255)');
  ...
  for (const width of [390, 320]) {
    ...
    await expect(cta).toBeInViewport();
    await expect(page.getByRole('navigation', { name: 'Main navigation' }).getByRole('link', { name: 'Open console' })).toBeInViewport();
...
test('portal example copies the displayed code and reports clipboard failures', async ({ page, context }) => {
  ...
  await expect(footer.getByRole('link', { name: 'SDK', exact: true })).toHaveAttribute('href', 'https://github.com/OyadotAI/oya-browser/tree/main/packages/sdk');
  await expect(footer.getByRole('link', { name: 'Desktop' })).toHaveAttribute('href', '/docs#download');
  await expect(footer.getByRole('link', { name: 'Self-host' })).toHaveAttribute('href', 'https://github.com/OyadotAI/oya-browser/blob/main/docs/self-hosting.md');
});
