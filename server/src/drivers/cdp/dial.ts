/**
 * Dialling a browser's CDP WebSocket. It lives with the CDP driver because the
 * driver is what owns an endpoint to dial; the gateway re-exports it.
 */
...
/** Where raw CDP reaches a browser at `url`: a fresh socket for each caller that opens it. */
export const endpointAt = (url: string) => ({ url, open: () => dialWithTimeout(url) });
