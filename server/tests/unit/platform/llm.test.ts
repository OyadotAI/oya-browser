...
describe('chatCompletion', () => {
  ...
  it('drops a trailing slash on the base URL, which doubles into a 404', async () => {
    const fetch = answer({ choices: [{ message: { content: 'hi' } }] });
    await chatCompletion({ baseUrl: 'https://api.test/v1/openai/', apiKey: 'sk-1', model: 'm', messages: [] });
    assert.equal(call(fetch).url, 'https://api.test/v1/openai/chat/completions');
  });
