...
describe('persona site routes', () => {
  ...
  it('lists which portals have a second factor, by kind, never the secret', async () => {
    const p = service.create(KEY);
    await mfa.set(p.id, { type: 'totp', secret: 'JBSWY3DPEHPK3PXP' }, 'portal.example.net');
    ...
    assert.deepEqual(res.body, { configured: false, sites: [{ domain: 'portal.example.net', type: 'totp' }] });
