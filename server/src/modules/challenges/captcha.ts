...
import { whenReady } from './page-ready.ts';
...
export async function handle(evaluate, { solve = true, providerSolves = false, env = process.env } = {}) {
  // A challenge on a page that has not finished loading is still a challenge;
  // reading too early is how a run walks past one and loops on the form behind it.
