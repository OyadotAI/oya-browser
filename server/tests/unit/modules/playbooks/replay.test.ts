...
describe('play across tabs', () => {
  ...
  describe('play without re-reading the page', () => {
    ...
    it('clicks a link by its own target without reading the page', async () => {
      // Both, as the analyzer records them: `href` resolved, `rawHref` as the markup wrote it.
      // Only the raw one can be matched by an attribute selector.
      const steps = [
        {
          action: 'click',
          el: {
            tag: 'a',
            href: 'https://en.wikipedia.org/wiki/Managed_care',
            rawHref: '/wiki/Managed_care',
            text: 'Managed care',
          },
        },
      ];
    ...
    it('does not build a selector from a resolved href, which the markup may not contain', async () => {
      // Hacker News writes its logo link as href="news.ycombinator.com"; the resolved value is
      // "https://news.ycombinator.com/". Selecting on the latter matched nothing, so every such
      // click quietly cost an extra page analysis and logged an error for a step that worked.
      const link: any = { id: 3, type: 'link', tag: 'a', text: 'Hacker News', visible: true };
      const steps = [
        { action: 'click', el: { tag: 'a', type: 'link', href: 'https://news.ycombinator.com/', text: 'Hacker News' } },
      ];
      answer = (action, params) =>
        action === 'analyze' ? { ok: true, data: { elements: [link] } } : page(action, params);
      await play(KEY, BROWSER, pb(steps), {});
      assert.equal(analyses(), 1, 'it reads the page instead of guessing a selector');
      assert.deepEqual(commands().find((c) => c.action === 'click').params, { selector: '[data-ac-id="3"]' });
    });
