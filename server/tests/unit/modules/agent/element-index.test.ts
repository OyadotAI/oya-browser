...
import { elementIndex, pageGuide, analysisText } from '../../../../src/modules/agent/element-index.ts';
...
describe('analysisText asked for no content', () => {
  const page = {
    format: 'markdown',
    facts: { url: 'https://a.test/', title: 'Shop' },
    blocks: [
      { region: 'main', kind: 'h1', text: 'Winter sale' },
      {
        region: 'main',
        kind: 'text',
        text: `Everything half price today. ${'More words about the sale. '.repeat(40)}`,
      },
      { id: 1, region: 'main', kind: 'link', text: 'Basket', target: '/basket', state: '' },
    ],
    elements: [{ id: 1, type: 'link', text: 'Basket', href: '/basket', visible: true }],
  };

  it('answers with the elements and leaves the page words out', () => {
    const text = analysisText(page, { content: false });
    assert.match(text, /Basket/);
    assert.doesNotMatch(text, /Everything half price today/);
    assert.doesNotMatch(text, /Winter sale/);
  });

  it('is shorter than the whole page, which is why an agent asks for it', () => {
    assert.ok(analysisText(page, { content: false }).length < analysisText(page).length);
  });

  it('still answers with the whole page by default', () => {
    assert.match(analysisText(page), /Everything half price today/);
  });
});
