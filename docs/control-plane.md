...
`POST /api/control/sessions/{id}/stop` saves profile state before cleanup. `{ "force": true }` permits cleanup despite a profile-save error. `/cancel` stops a session in any state and answers its final state; a browser this server holds is gone when the answer arrives. Recovery returns the original ready session when available. Otherwise `/recover` requires explicit `{ "replace": true }`, and only after the original is stopped or failed. For a plain CDP replacement, supply the new `wsUrl` explicitly. One endpoint drives one browser: a second start on an endpoint another browser already holds answers 409 `endpoint_in_use`, and the body names the holder's id when it is yours. Replacement preserves admitted policies and restores supported profile data; it cannot promise that the previous action did or did not run. Never automatically replay payments or other non-idempotent browser actions after `command_outcome_unknown`.
...
node --env-file=server/.env server/tests/control-supabase.test.js
# Optional isolated Postgres fixture with migration 008 applied:
PGHOST=/path/to/socket PGPORT=55439 node server/tests/control-postgres.test.js
# Real Docker/Electron contract:
OYA_TEST_IMAGE=oya-browser:managed node server/tests/managed-runtime.test.js
```
