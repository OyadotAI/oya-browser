...
const { validData, MAX_FILE_BYTES } = await import('./src/api.js');
const { UPLOAD_FILE_JS, dataKey } = await import('./src/chat-service.js');
const { sanitizeSteps, variablesOf, missingVariables, renderPlaywright } = await import('./src/playbook.js');
...
assert.ok(!validData({ r: { ...resume, b64: 'A'.repeat(Math.ceil(MAX_FILE_BYTES / 3) * 4 + 4) } }), `over ${MAX_FILE_BYTES} bytes is refused`);
...
try {
  ...
  assert.ok(!stale.ok && /no longer on the page/.test(stale.error), `a stale handle errors (got ${JSON.stringify(stale)})`);
